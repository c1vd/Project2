...
class Config:
    def __init__(self):
        self.no_scope_crosshair = False

    def update(self):
        if keyboard.is_pressed("page up") and not self.no_scope_crosshair:
            self.no_scope_crosshair = True
        if keyboard.is_pressed("page down") and self.no_scope_crosshair:
            self.no_scope_crosshair = False
...
class Offsets:
    ...
    def update(self):
        client_dict_classes = json.loads(requests.get("https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/client.dll.json").text)["client.dll"]["classes"]
        offsets_client_dll_dict = json.loads(requests.get("https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/offsets.json").text)["client.dll"]
        ...
        self.m_bDormant = client_dict_classes["CGameSceneNode"]["fields"]["m_bDormant"]
...
class Hacks:
    ...
    def no_scope_crosshair(self):
        pm.draw_circle_lines(self.width // 2, self.height // 2, 3, Colors.green)
...
class Cheat:
    ...
    def run(self):
        ...
        while not keyboard.is_pressed("end"):
            ...
            if self.config.no_scope_crosshair:
                self.cheats.no_scope_crosshair()
